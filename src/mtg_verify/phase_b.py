...
def verify_phase_b_run() -> int:
    ...
    commands = [
        _run(
            "uv run --no-sync mtg-engine verify-phase-a",
            environment={"PHASE_A_ARTIFACT_ROOT": str(phase_a_root)},
        ),
    ...
    mapping = json.loads(
        (ROOT / "automation/phase-b-test-mapping.json").read_text(encoding="utf-8")
    )
    ...
    passed = (
        ...
        and all(result["exit_code"] == 0 for result in commands)
    )
    ...
    result = {
        "schema_version": "phase-b-result-v1",
        "run_id": run_id,
        "commit": commit,
        "branch": branch,
        ...
        "decklist_sha256": _sha(ROOT / "docs/source/decklist.txt"),
        "transcript_approval_document_sha256": hashlib.sha256(
            json.dumps(
                json.loads(
                    (ROOT / "docs/audit/phase-b-golden-transcripts/APPROVALS.json").read_text(
                        encoding="utf-8"
                    )
                ),
                sort_keys=True,
