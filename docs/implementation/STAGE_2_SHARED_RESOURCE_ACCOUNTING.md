...
The seed `391730338978874520` repaired Turn-5 and legacy Turn-6 combat-damage witnesses are deliberate Stage 2 resource-legality corrections. Their old `sufficient_mana = false` expectations encoded the previous Treasure-blind resource model even though the same captured states were already classified as legally executable/full-table-kill and had finite production witnesses. The Stage 2 solver regression checks that the `{1}{R}` allocation contains exactly two paid mana units and that any Treasure allocation does not exceed the number of untapped Treasure permanents counted directly from the captured state. That is a physical-allocation cross-check; it is not an independent feasibility proof because the tracker and that allocation check share `solve_state_payment()`.

Independent legality for both captured 391 states is therefore checked separately through the production `ActionBroker` and `GameExecutor`. The broker must expose the required Treasure mana activation when funding is needed, must then expose `glint-horn:loot`, and execution must record the production Glint-Horn activation. The corrected current measurements are `sufficient_mana = true` with no mana blocker. Historical archived evidence is not modified.

The legacy seed `101` Turn-10 first-access state also changes its private state hash under Stage 2, but the diagnostic shows no game-path or resource-count change. The historical state and the Stage 2 state have the same hand, same battlefield permanents, same opponent life totals, same library size, same Turn-10 precombat-main priority window, and the same six available Island/Mountain mana sources. Historically, first access was observed after five lands had been tapped into floating `{R}{R}{R}{U}{U}`, leaving one Island untapped. Stage 2 recognizes the same line earlier in that priority window, before those mana activations: all six lands remain untapped and the floating pool is empty. This earlier recognition is expected because the authoritative resource model reasons directly over persistent mana sources instead of waiting for them to be converted into floating mana.
...
Repeated trials are summarized with median and p95 values. The harness also records corpus coverage evidence for the Stage 2 scenario classes. The harness mechanically derives 25 percent headroom figures from measured p95, but those figures are `PROVISIONAL_NOT_FINAL` and are not an acceptance threshold.

**Performance-governance boundary:** the Stage 2 benchmark result is a pre-optimization observation of the current implementation. Its measured values and mechanically derived headroom are evidence for later optimization work; they are not a regression budget, not a Stage 3 performance contract, not a definition of acceptable performance, and not justification for retaining the current cloning/deep-copy cost.
...
Before PR #101 is marked review-ready, the branch must contain the measured raw benchmark result and environment/command record as pre-optimization evidence, current Phase A and Phase B durable certifications for changed covered paths, and green normal CI. Historical pilot evidence must remain unchanged, and no pilot or full study may be executed.
