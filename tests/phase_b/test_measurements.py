...
def game(index: int, *, access: bool) -> GameMeasurement:
    return GameMeasurement(
        ...
        failure_labels={5: ("mana_shortage",), 6: ("action_density_shortage",), 8: (), 10: ()},
...
def test_measurements_preserve_all_raw_fields_and_exact_denominators() -> None:
    ...
    assert summary.game_denominator == 2 and summary.mode_denominators == {
        "EXPLORATORY": 1,
        "STANDARD": 1,
    }
    assert summary.checkpoint_access_numerators == {
        5: 0,
        6: 0,
        8: 1,
        10: 1,
    } and summary.checkpoint_access_denominators == {5: 2, 6: 2, 8: 2, 10: 2}
    assert (
        summary.combo_attempt_counts["dualcaster_twinflame"] == 1
        and summary.combo_kill_counts["dualcaster_twinflame"] == 1
    )
    assert (
        summary.earliest_legal_attempt_turn_counts == {8: 1}
        and summary.actual_first_attempt_turn_counts == {8: 1}
        and summary.terminal_turn_counts == {8: 1}
    )
    assert (
        summary.never_legal_attempt_count == 1
        and summary.never_attempted_count == 1
        and summary.future_information_rejections == 1
    )
    assert summary.raw_measurement_sha256 == measurement_digest(records) and measurement_digest(
        tuple(reversed(records))
    ) == measurement_digest(records)
