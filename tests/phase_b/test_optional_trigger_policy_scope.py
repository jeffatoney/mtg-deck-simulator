...
def test_replay_provider_ignores_explicit_optional_trigger_choices() -> None:
    choices = [
        {
            "kind": "OPTIONAL_TRIGGER",
            "selected": {
                "actor_id": "P0",
                "ability_id": "curiosity:damage",
                "effect_kind": "DRAW",
                "take": False,
        ...
        },
        {
            ...
            "selected": {
                ...
                "evaluator_id": "contextual_combo_v1",
                "evaluator_sha256": "1" * 64,
                "diagnostics": {"strategy": "TAKE_REVIEWED_OPTIONAL_TRIGGER"},
                "chosen_at": "RESOLUTION",
            },
        },
    ]
    provider = RecordedStrategicChoiceProvider(choices)

    selection = provider.choose_optional_trigger(_request("curiosity:damage", "DRAW"))

    assert selection.take is True
    assert selection.evaluator_id == "contextual_combo_v1"
    assert selection.diagnostics["strategy"] == "TAKE_REVIEWED_OPTIONAL_TRIGGER"
