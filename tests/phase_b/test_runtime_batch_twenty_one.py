...
class FixedCounterProvider:
    ...
    def choose_counter_payment(self, request: CounterPaymentRequest) -> CounterPaymentSelection:
        ...
        return CounterPaymentSelection(
            self.outcome,
            "runtime-batch-twenty-one-provider",
            "0" * 64,
            {"requested_outcome": self.outcome},
        )
...
def test_syncopate_uses_x_for_payment_and_exiles_when_declined() -> None:
    ...
    executor.pass_priority("P0")
    before = state_hash(state)
    with pytest.raises(IllegalAction, match="unavailable counter-payment outcome"):
        executor.pass_priority("P1")
    assert state_hash(state) == before
    assert state.players["P1"].mana_pool["C"] == 2
    assert len(state.stack) == 2
