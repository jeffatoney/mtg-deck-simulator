...
def test_phase_b_mapping_has_no_slice_placeholders_and_covers_all_blockers() -> None:
    ...
    assert not any("PENDING_SLICE" in node for nodes in mapping["requirements"].values() for node in nodes)
...
def test_verifier_and_durable_certification_fail_closed_on_real_blockers() -> None:
    ...
    assert "Build Phase B certification candidate" in workflow and "Durable Phase B certification is current" in workflow
